    ...
    if (filterBy) {
        if (filterBy.byUserId) {
            entities = entities.filter((entity) => {
                return entity.createdBy._id === filterBy.byUserId || entity.members.some((member) => member._id === filterBy.byUserId);
            });
        }
...
function postMany(entityType, newEntities) {
    return query(entityType).then((entities) => {
        newEntities = newEntities.map((entity) => ({ ...entity, _id: entity._id === 'u101' ? 'u101' : _makeId() }));
