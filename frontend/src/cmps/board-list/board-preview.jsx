...
class _BoardPreview extends React.Component {
  ...
  bgcs = [
    ...
    },
    {
      ...
      title:
        'abstract 3d illustration of geometrical objects with abstract light scattering on them.',
    },
  ...
  onCreateNewBoard = async () => {
    ...
    this.state.markedId < 4
      ? (boardToAdd.style.imgUrl = this.bgcs[this.state.markedId].bgc.substring(
          0,
          this.bgcs[this.state.markedId].bgc.length - 6
        ))
          ...
          <Modal showCloseIcon={true} open={this.state.isModal} onClose={this.onCloseModal}>
            <div className="new-board">
              {this.state.markedId < 4 && (
                <div
                  className="main img"
                  style={{ backgroundImage: `url(${this.bgcs[this.state.markedId].bgc})` }}></div>
              )}
              {this.state.markedId > 3 && (
                <div
                  className="main"
                  style={{ backgroundColor: this.bgcs[this.state.markedId].bgc }}></div>
              )}
              <input
                autoComplete="off"
                autoCorrect="off"
